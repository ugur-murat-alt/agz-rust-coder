...
const ENV_PREFIX: &str = "AGZ_RUST_MCP_";
...
fn platform_cache_dir() -> PathBuf {
    if let Some(base) = env::var_os("XDG_CACHE_HOME") {
        return PathBuf::from(base).join("agz-rust-mcp");
    }
    if let Some(home) = env::var_os("HOME") {
        return PathBuf::from(home).join(".cache").join("agz-rust-mcp");
    }
    env::temp_dir().join("agz-rust-mcp")
}
...
mod tests {
    ...
    fn test_path(suffix: &str) -> PathBuf {
        ...
        fs::canonicalize(env::temp_dir())
            ...
            .join(format!(
                "agz-rust-mcp-config-{stamp}-{}-{suffix}",
                std::process::id()
            ))
    }
    ...
    fn precedence_is_cli_over_environment_over_toml_over_defaults() {
        ...
        let config = Config::from_sources(
            ...
            [("AGZ_RUST_MCP_GATE__HARD_TIMEOUT_MS", "3000")],
            &cli,
        )
        .unwrap();
    ...
    fn profile_config_accepts_toml_environment_and_cli_layers() {
        ...
        let config = Config::from_sources(
            ...
            [(
                "AGZ_RUST_MCP_PROFILE__RUNTIME_RUN_TIMEOUT_MS",
                "9000",
            ), ("AGZ_RUST_MCP_PROFILE__MAX_RUNS", "5")],
            &cli,
        )
        .unwrap();
    ...
    fn repair_config_accepts_toml_environment_and_cli_layers() {
        ...
        let config = Config::from_sources(
            ...
            [
                ("AGZ_RUST_MCP_REPAIR__MAX_CANDIDATES", "3"),
                ("AGZ_RUST_MCP_REPAIR__MINIMIZE_MAX_COMPILES", "7"),
            ],
            &cli,
        )
        .unwrap();
        ...
        let environment_out_of_range = Config::from_sources(
            "/workspace",
            None,
            [("AGZ_RUST_MCP_REPAIR__MAX_COMPILES", "0")],
            &plain_cli,
        );
        assert!(matches!(
    ...
    fn work_config_accepts_toml_environment_and_cli_layers() {
        ...
        let config = Config::from_sources(
            "/workspace",
            Some("[tools]\nwork = false\n[work]\nmax_handoffs = 1\n"),
            [("AGZ_RUST_MCP_WORK__MAX_CANDIDATES", "2")],
            &cli,
        )
        .unwrap();
    ...
    fn list_layers_replace_instead_of_append() {
        ...
        let config = Config::from_sources(
            test_path("default"),
            Some(&toml),
            [("AGZ_RUST_MCP_SERVER__ALLOW_ROOTS", environment)],
            &cli,
        )
        .unwrap();
    ...
    fn verify_configuration_uses_the_declared_precedence() {
        ...
        let config = Config::from_sources(
            ...
            Some("[verify]\nmax_cells = 2\nmax_wall_ms = 5000\nrepeats = 3\nmax_tests = 5\n"),
            [
                ("AGZ_RUST_MCP_VERIFY__MAX_CELLS", "4"),
                ("AGZ_RUST_MCP_VERIFY__REPEATS", "4"),
                ("AGZ_RUST_MCP_TOOLS__VERIFY", "true"),
            ],
            &cli,
        )
    ...
    fn malformed_environment_is_not_silently_ignored() {
        let result = Config::from_sources(
            "/workspace",
            None,
            [("AGZ_RUST_MCP_GATE__HOST_CONCURRENCY", "many")],
            &cli(),
        );
        assert!(matches!(
            result,
    ...
    fn context_configuration_wires_toml_environment_and_cli() {
        ...
        let config = Config::from_sources(
            ...
            Some("[context]\nmax_capsules = 4\ncapsule_ttl_ms = 5000\nmax_items = 8\n"),
            [
                ("AGZ_RUST_MCP_CONTEXT__MAX_CAPSULES", "6"),
                ("AGZ_RUST_MCP_CONTEXT__MAX_ITEMS", "12"),
                ("AGZ_RUST_MCP_CONTEXT__CAPSULE_TTL_MS", "7000"),
            ],
            &cli,
        )
    ...
    fn api_configuration_wires_toml_environment_and_cli() {
        ...
        let config = Config::from_sources(
            ...
            [
                ("AGZ_RUST_MCP_API__MAX_SNIPPETS", "1"),
                ("AGZ_RUST_MCP_API__COMPILE_TIMEOUT_MS", "7000"),
            ],
            &cli,
        )
    ...
    fn non_unicode_environment_is_filtered_or_reported_without_panicking() {
        ...
        let relevant = OsString::from("AGZ_RUST_MCP_GATE__HOST_CONCURRENCY");
