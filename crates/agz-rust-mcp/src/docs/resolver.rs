...
pub const DOCS_USER_AGENT: &str = "agz-rust-mcp rust.docs (advisory)";
...
fn windows_cargo_target_argument(target: &Path) -> Result<PathBuf, String> {
    // Cargo and the linker need an ordinary Win32 spelling for canonical
    // (verbatim) paths; reject only when no identity-preserving form exists.
    crate::process::win32_spelling(target)
        .ok_or_else(|| "Cargo output path has no identity-preserving Win32 spelling".to_owned())
}
...
fn fingerprint(
    ...
    hasher.update(b"agz-rust-mcp-docs-v1\0");
    let mut total = 0u64;
...
fn resolve_cache_root(workspace_root: &Path, requested: Option<&Path>) -> PathBuf {
    ...
    let fallback = absolute_path(&std::env::temp_dir().join("agz-rust-mcp").join("docs"));
    if fallback.starts_with(workspace_root) {
        absolute_path(
            &std::env::temp_dir().join(format!("agz-rust-mcp-docs-{}", std::process::id())),
        )
    } else {
        fallback
    }
