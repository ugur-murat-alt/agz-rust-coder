...
const TARGET_ENV: &str = "AGZ_RUST_MCP_GUARD_TARGET";
#[cfg(unix)]
const DEVICE_ENV: &str = "AGZ_RUST_MCP_GUARD_DEVICE";
#[cfg(unix)]
const INODE_ENV: &str = "AGZ_RUST_MCP_GUARD_INODE";
/// Enables the LSP-only stable root descriptor in the guarded child.
const LSP_ROOT_DESCRIPTOR_ENV: &str = "AGZ_RUST_MCP_GUARD_LSP_ROOT_DESCRIPTOR";
#[cfg(unix)]
const LSP_ROOT_DESCRIPTOR_FD: i32 = 198;
const TEST_READY_ENV: &str = "AGZ_RUST_MCP_GUARD_TEST_READY";
const TEST_CONTINUE_ENV: &str = "AGZ_RUST_MCP_GUARD_TEST_CONTINUE";

#[derive(Debug, thiserror::Error)]
pub enum RootBindingError {
    #[error("cannot resolve the agz-rust-mcp root guard executable: {0}")]
...
fn guard_executable() -> Result<PathBuf, RootBindingError> {
    ...
    if current
        .file_stem()
        .is_some_and(|name| name == "agz-rust-mcp")
    {
    ...
    let sibling = parent.join(format!("agz-rust-mcp{}", env::consts::EXE_SUFFIX));
    if sibling.is_file() {
