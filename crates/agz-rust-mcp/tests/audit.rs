...
#[cfg(unix)]
use agz_rust_mcp::tools::audit::InvalidPathReason;
use agz_rust_mcp::tools::audit::{
    AuditCancellation, AuditError, AuditFinding, AuditLimits, AuditRequest, AuditService,
    AuditSkipReason, AuditSummary,
};
use agz_rust_mcp::workspace::{ClientRoots, RootGuard, WorkspaceRoot};
...
impl TestRoot {
    fn new(label: &str) -> Self {
        ...
        let path = fs::canonicalize(std::env::temp_dir())
            ...
            .join(format!(
                "agz-rust-mcp-audit-{label}-{}-{stamp}-{sequence}",
                std::process::id()
            ));
...
fn cancellation_has_a_distinct_reason_and_fails_before_scanning() {
    ...
    request.cancel();
    let cancellation = agz_rust_mcp::tools::audit::AuditCancellation::new(
        request,
    ...
    assert!(matches!(
        ...
        Err(AuditError::Cancelled(
            agz_rust_mcp::tools::audit::AuditCancellationReason::Request
        ))
    ));
}
...
fn root_epoch_cancellation_has_its_own_reason() {
    ...
    assert!(matches!(
        result,
        Err(AuditError::Cancelled(
            agz_rust_mcp::tools::audit::AuditCancellationReason::RootEpoch
        ))
    ));
}
...
async fn async_scan_runs_on_the_blocking_pool_and_observes_shutdown_cancellation() {
    ...
    assert!(matches!(
        result,
        Err(AuditError::Cancelled(
            agz_rust_mcp::tools::audit::AuditCancellationReason::Shutdown
        ))
    ));
}
