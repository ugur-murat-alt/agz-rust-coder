...
use agz_rust_mcp::lsp::client::default_server_request;
use agz_rust_mcp::{
...
fn mock_binary() -> &'static PathBuf {
    MOCK_BINARY.get_or_init(|| {
        ...
        let output_dir =
            temp_dir().join(format!("agz-rust-mcp-mock-fixture-{}", std::process::id()));
...
fn log_path(label: &str) -> PathBuf {
    let path = temp_dir().join(format!(
        "agz-rust-mcp-lsp-client-{label}-{}-{}.log",
        std::process::id(),
...
async fn closed_documents_release_their_per_document_lock_budget() {
    let client = spawn_client("echo", None).await;
    for index in 0..(agz_rust_mcp::lsp::client::DEFAULT_MAX_DOCUMENTS + 8) {
