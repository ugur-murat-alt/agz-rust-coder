...
mod linux_process_supervisor {
    ...
    async fn authorized_launch_rejects_a_guard_binary_inside_its_root() {
        let guard = PathBuf::from(env!("CARGO_BIN_EXE_agz-rust-mcp"));
        let root = guard
            .parent()
    ...
    async fn root_contained_executable_uses_the_verified_directory_after_replacement() {
        ...
        run_options.env.insert(
            OsString::from("AGZ_RUST_MCP_GUARD_TEST_READY"),
            ready.as_os_str().to_owned(),
        );
        run_options.env.insert(
            OsString::from("AGZ_RUST_MCP_GUARD_TEST_CONTINUE"),
...
mod unix_process_journal {
    ...
    use agz_rust_mcp::process::{JournalError, ProcessJournal};
...
fn process_module_keeps_portable_cfg_boundaries() {
    let _ = agz_rust_mcp::process::ProcessSupervisor::without_journal();
    let _ = agz_rust_mcp::process::ProcessGroupIdentity::Unmanaged;
}
