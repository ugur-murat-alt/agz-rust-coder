...
impl TestRoot {
    fn new(label: &str) -> Self {
        ...
        let path = fs::canonicalize(std::env::temp_dir())
            ...
            .join(format!(
                "agz-rust-mcp-diagnostics-{label}-{}-{nonce}",
                std::process::id()
            ));
...
fn suggestion_sources_larger_than_the_snapshot_budget_are_rejected() {
    ...
    file.set_len(agz_rust_mcp::diagnostics::MAX_SOURCE_SNAPSHOT_BYTES + 1)
        .unwrap();
    ...
    assert_eq!(
        ...
        agz_rust_mcp::diagnostics::MAX_SOURCE_SNAPSHOT_BYTES + 1
    );
}
