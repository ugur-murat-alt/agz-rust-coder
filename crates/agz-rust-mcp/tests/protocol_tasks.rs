...
fn spawn_local_docs_server(
    ...
    let task = tokio::spawn(async move {
        ...
        let _state = state;
        config.docs.fallback = agz_rust_mcp::config::DocsFallback::Local;
...
fn client_info(capabilities: ClientCapabilities) -> ClientInfo {
    ClientInfo::new(
        capabilities,
        Implementation::new("agz-rust-mcp-task-test", "0.1.0"),
    )
}
...
async fn cancelling_a_docs_task_stops_the_real_local_cargo_process() -> Result<()> {
    ...
    let root = temp.join(format!(
        "agz-rust-mcp-docs-task-{}-{stamp}",
        std::process::id()
    ));
    let cache = temp.join(format!(
        "agz-rust-mcp-docs-task-cache-{}-{stamp}",
        std::process::id()
    ));
