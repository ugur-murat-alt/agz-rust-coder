...
#[cfg(unix)]
use agz_rust_mcp::workspace::StdGitProbe;
...
impl GitProbe for FakeGit {
    ...
    ) -> Result<GitOutput, agz_rust_mcp::workspace::IdentityError> {
...
impl TestDir {
    fn new(label: &str) -> Self {
        ...
        let path = fs::canonicalize(std::env::temp_dir())
            ...
            .join(format!("agz-rust-mcp-identity-{label}-{stamp}"));
...
fn workspace(root: &TestDir) -> agz_rust_mcp::workspace::WorkspaceRoot {
    let guard = Arc::new(
...
fn identity_input<'a>(
    root: &'a TestDir,
    workspace: &'a agz_rust_mcp::workspace::WorkspaceRoot,
    git: &'a FakeGit,
