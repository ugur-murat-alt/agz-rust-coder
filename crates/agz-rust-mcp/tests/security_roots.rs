...
#[cfg(windows)]
use agz_rust_mcp::lsp::NormalizeError;
use agz_rust_mcp::lsp::path_to_file_uri;
use agz_rust_mcp::workspace::{
    ClientRoots, RootError, RootGuard, WalkLimits, WorkspaceRoot, parse_file_uri,
};

#[cfg(unix)]
use agz_rust_mcp::workspace::WalkIssueKind;
...
impl TestDir {
    fn new(label: &str) -> Self {
        ...
        let path = fs::canonicalize(std::env::temp_dir())
            ...
            .join(format!("agz-rust-mcp-roots-{label}-{stamp}"));
...
fn dependency_roots_are_not_selectable_workspace_roots() {
    ...
    assert_eq!(
        resolved.root.kind(),
        agz_rust_mcp::workspace::RootKind::Dependency
    );
}
