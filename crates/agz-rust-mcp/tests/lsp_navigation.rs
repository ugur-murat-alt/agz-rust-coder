...
fn requested_authority(root: &TestRoot) -> std::sync::Arc<agz_rust_mcp::workspace::AuthorizedRoot> {
    let guard =
...
fn replace_root(root: &TestRoot, label: &str) -> PathBuf {
    let old = root.path().with_file_name(format!(
        "agz-rust-mcp-navigation-{label}-old-{}",
        std::process::id()
    ));
